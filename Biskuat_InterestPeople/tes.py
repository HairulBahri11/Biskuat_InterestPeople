...
model = load_model(os.path.join(BASE_DIR, 'model4.hdf5'))
...
classes = ['ANIMAL', 'FOOD', 'OTOMOTIF', 'TRAVELING']
...
def dashboard():
    ...
    cur.execute(
        "SELECT COUNT(*) as total FROM load_gambar WHERE user_id = %s AND klasifikasi ='ANIMAL' AND instagram_id =%s", parameter)
    animal = cur.fetchone()

    return render_template('success.html', data=fetchdata,  food=food,  otomotif=otomotif, traveling=traveling, animal=animal)
    # else:
